...
// Funções para lidar com caminhos em diferentes ambientes de jailbreak
static NSString *getVCamBasePath() {
    // Para jailbreak rootless (palera1n)
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/var/jb"]) {
        return @"/var/jb/var/tmp";
    }
    
    // Para jailbreak rootful (unc0ver, etc.)
    else if ([[NSFileManager defaultManager] fileExistsAtPath:@"/var/lib/dpkg"]) {
        return @"/tmp";
    }
    
    // Fallback para o diretório de cache do usuário se nenhum for encontrado
    else {
...
static NSString *getVCamLogPath() {
    return [getVCamBasePath() stringByAppendingPathComponent:@"vcam_debug.log"];
}
...
// Agora a função ensureDirectoryPermissions
static BOOL ensureDirectoryPermissions(NSString *path) {
    BOOL isDirectory = NO;
    ...
    if (!exists) {
        ...
        if (![[NSFileManager defaultManager] createDirectoryAtPath:path
                                     withIntermediateDirectories:YES
                                                      attributes:nil
                                                           error:&error]) {
            vcam_logf(@"Erro ao criar diretório %@: %@", path, error);
            return NO;
        }
        vcam_log(@"Diretório criado com sucesso");
    } else if (!isDirectory) {
        vcam_logf(@"Caminho existe mas não é um diretório: %@", path);
        return NO;
    }
...
// Caminho do arquivo de vídeo padrão - será inicializado no %ctor
static NSString *g_videoFile = nil;
...
- (void)logSampleBufferInfo:(CMSampleBufferRef)sampleBuffer withPrefix:(NSString *)prefix;
@end
...
// Método auxiliar para diagnóstico de buffers de vídeo
- (void)logSampleBufferInfo:(CMSampleBufferRef)sampleBuffer withPrefix:(NSString *)prefix {
    if (!sampleBuffer) {
        vcam_logf(@"%@: Buffer nulo", prefix);
        return;
    }
    
    if (!CMSampleBufferIsValid(sampleBuffer)) {
        vcam_logf(@"%@: Buffer inválido", prefix);
        return;
    }
    ...
    if (formatDesc) {
        ...
        vcam_logf(@"%@: MediaType: %4.4s, SubType: %4.4s",
                 prefix,
                 (char*)&mediaType,
                 (char*)&mediaSubType);
    }
    
    // Informações do timing
    CMTime pts = CMSampleBufferGetPresentationTimeStamp(sampleBuffer);
    CMTime duration = CMSampleBufferGetDuration(sampleBuffer);
    vcam_logf(@"%@: PTS: %.3fs, Duration: %.3fs",
             prefix,
             CMTimeGetSeconds(pts),
             CMTimeGetSeconds(duration));
    
    // Informações do pixel buffer (para vídeo)
    ...
    if (pixelBuffer) {
        ...
        vcam_logf(@"%@: Dimensões: %zux%zu, Formato de pixel: %4.4s",
                 prefix,
                 width,
                 height,
                 (char*)&pixelFormat);
...
- (BOOL)setupVideoReader {
    @try {
        ...
        // Verificamos os atributos do arquivo
        NSError *attributeError = nil;
        NSDictionary *attributes = [g_fileManager attributesOfItemAtPath:g_videoFile error:&attributeError];
        if (attributeError) {
            vcam_logf(@"Erro ao ler atributos do arquivo: %@", attributeError);
        } else {
            vcam_logf(@"Tamanho do arquivo: %lld bytes", [attributes fileSize]);
            
            // Se o arquivo for muito pequeno, provavelmente está corrompido ou vazio
            if ([attributes fileSize] < 1000) {
                vcam_log(@"AVISO: Arquivo de vídeo parece muito pequeno ou vazio");
            }
        }
...
- (void)checkAndRestartReaderIfNeeded {
    if (_reader && _reader.status == AVAssetReaderStatusCompleted) {
        ...
        if (![self setupVideoReader]) {
            vcam_log(@"⚠️ Falha ao reiniciar o leitor de vídeo");
            // Força a reinicialização completa na próxima chamada
            g_bufferReload = YES;
        }
    } else if (_reader && _reader.status == AVAssetReaderStatusFailed) {
        vcam_logf(@"⚠️ Erro no leitor de vídeo: %@", _reader.error);
        [self releaseResources];
        if (![self setupVideoReader]) {
            vcam_log(@"⚠️ Falha ao reiniciar o leitor após erro");
            g_bufferReload = YES;
        }
    }
}
...
- (CMSampleBufferRef _Nullable)getCurrentFrame:(CMSampleBufferRef _Nullable)originSampleBuffer forceReNew:(BOOL)forceReNew {
    ...
    dispatch_sync(_processingQueue, ^{
        ...
        if (!videoTrackout_32BGRA_Buffer) {
            vcam_log(@"ERRO: Falha ao copiar buffer 32BGRA");
        }
        ...
        if (!videoTrackout_420YpCbCr8BiPlanarVideoRange_Buffer) {
            vcam_log(@"ERRO: Falha ao copiar buffer 420 Video Range");
        }
        ...
        if (!videoTrackout_420YpCbCr8BiPlanarFullRange_Buffer) {
            vcam_log(@"ERRO: Falha ao copiar buffer 420 Full Range");
        }

        // Caso nenhum buffer seja obtido, tentar recarregar o vídeo
        if (!videoTrackout_32BGRA_Buffer && !videoTrackout_420YpCbCr8BiPlanarVideoRange_Buffer && !videoTrackout_420YpCbCr8BiPlanarFullRange_Buffer) {
            vcam_log(@"ERRO CRÍTICO: Todos os buffers falharam, recarregando vídeo");
            g_bufferReload = YES;
            [self releaseResources];
            if (![self setupVideoReader]) {
                vcam_log(@"Falha ao reconfigurar leitor de vídeo após erros");
            }
            result = NULL;
            return;
        }
        
        // Log diagnóstico para os buffers
        if (videoTrackout_32BGRA_Buffer) {
            [self logSampleBufferInfo:videoTrackout_32BGRA_Buffer withPrefix:@"32BGRA"];
        } else {
            vcam_log(@"32BGRA: Falha ao obter buffer");
        }
        
        if (videoTrackout_420YpCbCr8BiPlanarVideoRange_Buffer) {
            [self logSampleBufferInfo:videoTrackout_420YpCbCr8BiPlanarVideoRange_Buffer withPrefix:@"420V"];
        } else {
            vcam_log(@"420V: Falha ao obter buffer");
        }
        
        if (videoTrackout_420YpCbCr8BiPlanarFullRange_Buffer) {
            [self logSampleBufferInfo:videoTrackout_420YpCbCr8BiPlanarFullRange_Buffer withPrefix:@"420F"];
        } else {
            vcam_log(@"420F: Falha ao obter buffer");
        }
        ...
        switch(subMediaType) {
            ...
            case kCVPixelFormatType_420YpCbCr8BiPlanarFullRange:
                ...
                break;
            default:
                vcam_logf(@"Formato não reconhecido (%d), usando formato disponível", (int)subMediaType);
                // Tenta usar qualquer buffer disponível
                if (videoTrackout_420YpCbCr8BiPlanarFullRange_Buffer) {
                    CMSampleBufferCreateCopy(kCFAllocatorDefault, videoTrackout_420YpCbCr8BiPlanarFullRange_Buffer, &newsampleBuffer);
                } else if (videoTrackout_32BGRA_Buffer) {
                    CMSampleBufferCreateCopy(kCFAllocatorDefault, videoTrackout_32BGRA_Buffer, &newsampleBuffer);
                } else if (videoTrackout_420YpCbCr8BiPlanarVideoRange_Buffer) {
                    CMSampleBufferCreateCopy(kCFAllocatorDefault, videoTrackout_420YpCbCr8BiPlanarVideoRange_Buffer, &newsampleBuffer);
                }
        }
...
// Método atualizado para obter a janela principal da aplicação (iOS 16 compatível)
+(UIWindow*)getKeyWindow{
    vcam_log(@"GetFrame::getKeyWindow - Buscando janela principal");
    
    UIWindow *keyWindow = nil;
    
    // Abordagem baseada em cenas para iOS 13+
    ...
    // Método alternativo se não encontramos usando a abordagem acima
    if (!keyWindow) {
    ...
    if (keyWindow) {
        ...
        vcam_log(@"Falha ao encontrar janela principal");
    }
...
- (void)addSublayer:(CALayer *)layer{
    ...
    if (![[self sublayers] containsObject:g_previewLayer]) {
        ...
        // Cria a camada de preview de amostra de buffer - VERSÃO CORRIGIDA
        g_previewLayer = [[AVSampleBufferDisplayLayer alloc] init];
        
        // Configurações importantes para exibição de vídeo
        g_previewLayer.videoGravity = AVLayerVideoGravityResizeAspect;  // Mudado para ResizeAspect
        g_previewLayer.opaque = YES;  // Torna a camada opaca para melhor performance
        [g_previewLayer setActions:@{@"opacity": [NSNull null]}];  // Desativa animações para mudanças de opacidade
        
        // Máscara preta para cobrir a visualização original
        g_maskLayer = [CALayer new];
        g_maskLayer.backgroundColor = [UIColor blackColor].CGColor;
        g_maskLayer.opacity = 0; // Começa invisível
        [g_maskLayer setActions:@{@"opacity": [NSNull null]}];  // Desativa animações para mudanças de opacidade
        
        // Verificação do estado da camada de visualização
        vcam_logf(@"Estado inicial da camada de visualização: %@",
                 g_previewLayer.readyForMoreMediaData ? @"Pronta" : @"Não pronta");
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            // Define o frame para cobrir toda a área de visualização
            g_previewLayer.frame = self.bounds;  // Use os limites da camada atual em vez da janela
            g_maskLayer.frame = self.bounds;
            vcam_logf(@"Tamanho das camadas inicializado: %@", NSStringFromCGRect(self.bounds));
            
            // Verifica novamente o estado após definir o frame
            vcam_logf(@"Estado da camada após inicialização: %@",
                     g_previewLayer.readyForMoreMediaData ? @"Pronta" : @"Não pronta");
        });
    }
}
...
-(void)step:(CADisplayLink *)sender{
    ...
    if (g_cameraRunning && g_previewLayer != nil) {
        ...
        if (nowTime - g_refreshPreviewByVideoDataOutputTime > 1000) {
            ...
            if (nowTime - refreshTime > 1000 / 30) {
                refreshTime = nowTime;
                
                // VERIFICAÇÃO IMPORTANTE: testa se a camada está pronta para receber mais dados
                if (!g_previewLayer.readyForMoreMediaData) {
                    vcam_log(@"⚠️ Layer não está pronta para mais dados!");
                    [g_previewLayer flush]; // Limpa buffer pendente
                    return;
                }
                ...
                if (newBuffer != nil) {
                    ...
                    // Log do buffer antes de enfileirar
                    CVImageBufferRef pixelBuffer = CMSampleBufferGetImageBuffer(newBuffer);
                    if (pixelBuffer) {
                        vcam_logf(@"Enfileirando frame de vídeo: %ldx%ld",
                                 CVPixelBufferGetWidth(pixelBuffer),
                                 CVPixelBufferGetHeight(pixelBuffer));
                    } else {
                        vcam_log(@"AVISO: Buffer de vídeo não tem imagem associada");
                    }
                    
                    // Enfileira o buffer diretamente - MODIFICADO para evitar cópias extras
                    [g_previewLayer enqueueSampleBuffer:newBuffer];
                    vcam_log(@"Frame enfileirado para exibição");
                } else {
                    vcam_log(@"⚠️ Não foi possível obter um frame válido");
                }
            }
        }
    }
...
// Função para configurar o hook do delegado do VideoDataOutput - atualizado para iOS 16
static void UpdateSampleBufferDelegateHook(id sampleBufferDelegate) {
    // Obtém a classe do delegado
    ...
    // Só continua se o método existir
    if (!originalMethod) {
        vcam_log(@"Método não encontrado na classe delegada, pulando hook");
        return;
    }
    
    // Armazena a implementação original
    __block IMP originalIMP = method_getImplementation(originalMethod);
    
    // Cria um bloco para a nova implementação
    IMP newIMP = imp_implementationWithBlock(^(id self, AVCaptureOutput *output, CMSampleBufferRef sampleBuffer, AVCaptureConnection *connection) {
        ...
        if ([g_fileManager fileExistsAtPath:g_videoFile]) {
            ...
            if (newBuffer != nil && g_previewLayer != nil && g_previewLayer.readyForMoreMediaData) {
                ...
                // Enfileira o buffer na camada de visualização
                [g_previewLayer enqueueSampleBuffer:newBuffer];
                vcam_log(@"Frame enfileirado pelo delegate");
            }
            ...
            ((void (*)(id, SEL, AVCaptureOutput *, CMSampleBufferRef, AVCaptureConnection *))originalIMP)(
                self, selector, output, newBuffer != nil ? newBuffer : sampleBuffer, connection
            );
            return;
        }
        ...
        ((void (*)(id, SEL, AVCaptureOutput *, CMSampleBufferRef, AVCaptureConnection *))originalIMP)(
            self, selector, output, sampleBuffer, connection
        );
    });
    
    // Substitui a implementação
    method_setImplementation(originalMethod, newIMP);
    vcam_log(@"Hook instalado com sucesso para a classe delegada");
}
...
- (void)setSampleBufferDelegate:(id<AVCaptureVideoDataOutputSampleBufferDelegate>)sampleBufferDelegate queue:(dispatch_queue_t)sampleBufferCallbackQueue{
    ...
    if (![hooked containsObject:className]) {
        ...
        // Usa o método de hook atualizado para iOS 16
        UpdateSampleBufferDelegateHook(sampleBufferDelegate);
    }
...
%ctor {
    ...
    // Garante que o diretório base exista e tenha permissões corretas
    NSString *basePath = getVCamBasePath();
    if (ensureDirectoryPermissions(basePath)) {
        vcam_log(@"Diretório base configurado com sucesso");
    } else {
        vcam_log(@"AVISO: Falha ao configurar diretório base");
    }
    
    // Inicializa caminhos de arquivos
    g_videoFile = [basePath stringByAppendingPathComponent:@"default.mp4"];
    vcam_logf(@"Caminho do vídeo definido como: %@", g_videoFile);
